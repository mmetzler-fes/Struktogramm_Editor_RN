	...
	while (currentNode && currentNode !== stopNode) {
		...
		if (successors.length === 2) {
			if (nodeType === 'loop' || nodeType === 'terminal' || ['for_loop', 'while_loop', 'repeat_loop'].includes(nodeType)) {
				...
				if (isStandardDecision || nodeType === 'if_else' || nodeType === 'decision') {
					const mergeNode = findMergeNode(G, successors[0], successors[1], currentNode);
					let yesNode, noNode;
					const label0 = (G.getEdgeData(currentNode, successors[0]).label || '').toLowerCase();

					if (label0.includes('ja') || label0.includes('yes') || label0.includes('true')) {
						yesNode = successors[0]; noNode = successors[1];
					} else if (label0.includes('nein') || label0.includes('no') || label0.includes('false')) {
						yesNode = successors[1]; noNode = successors[0];
					} else {
						// Fallback: Assume order Yes, No (as generated by convertTreeToGraph)
						yesNode = successors[0]; noNode = successors[1];
					}
